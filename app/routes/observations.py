...
def create_observation(fhir_resource: dict, response: Response, db: Session = Depends(get_db)):
...
def search_observations(
    ...
    if category:
        query = query.filter(Observation.category.contains([{"coding": [{"code": category}]}]))
    ...
    if date:
        ...
        query = query.filter(Observation.effective_datetime == datetime.fromisoformat(date))
...
def update_observation(observation_id: str, fhir_resource: dict, db: Session = Depends(get_db)):
